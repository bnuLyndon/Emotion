...
class TAGM(Recurrent):
    ...
    def __init__(self, output_dim,
        ...
        super(TAGM, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        self.W_t = self.add_weight((input_dim, self.output_dim * 1),
                                 initializer=self.init,
                                 name='{}_Wt'.format(self.name),
                                 regularizer=self.W_regularizer)
        self.U_t = self.add_weight((self.output_dim * 1, self.output_dim * 1),
                                 initializer=self.inner_init,
                                 name='{}_Ut'.format(self.name),
                                 regularizer=self.U_regularizer)
        self.b_t = self.add_weight((self.output_dim * 1,),
                                 initializer='zero',
                                 name='{}_bt'.format(self.name),
                                 regularizer=self.b_regularizer)  
                                                                
        self.M = self.add_weight((self.output_dim * 1, 1),
                                 initializer=self.inner_init,
                                 name='{}_M'.format(self.name),
                                 regularizer=self.U_regularizer)
        self.b_a = self.add_weight((1,),
                                 initializer='zero',
    ...
    def reset_states(self):
        ...
        if hasattr(self, 'states'):
            ...
            K.set_value(self.states[1],
                        np.zeros((input_shape[0], self.output_dim * 1)))
        else:
            self.states = [K.zeros((input_shape[0], self.output_dim)),
                           K.zeros((input_shape[0], self.output_dim * 1))]
    ...
    def step(self, x, states):
        ...
        if self.consume_less == 'cpu':
            ...
            h_t = K.dot(x * B_W, self.W_t) + self.b_t

        sig=activations.get('sigmoid')
        att = self.activation(h_t + K.dot(prev_att * B_U, self.U_t))
        alpha=sig(K.dot(att, self.M)+self.b_a)

        output = self.activation(alpha * h + (1-alpha) * K.dot(prev_output * B_U, self.U))
        return output, [output,att]
    ...
    def get_config(self):
        ...
        base_config = super(TAGM, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
